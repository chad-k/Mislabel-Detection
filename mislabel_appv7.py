# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 11:50:17 2026

@author: chad
Questions/issues: chad@hertzler.com
"""
...
with st.expander("Help / How to use this dashboard", expanded=False):
    st.markdown(
        """
### What this app does
This app detects **mislabeled or anomalous parts within each product type** using machine learning.

It works by:
1. **Separating data by Part Number** (e.g., PN-A, PN-B, PN-C)
2. **Analyzing each type independently** using 3+ numeric features (measurements, parameters, etc.)
3. **Flagging unusual parts** that don't match their declared type's typical pattern
4. **Explaining which features** made each part suspicious

### Smart Contamination Detection
Instead of using a fixed 10% threshold for all part types, this app **intelligently detects the optimal anomaly rate for each type**:

- **Tight-tolerance parts** (PN-A, precision): 3-5% contamination
  - Few natural variations, so even small deviations are unusual
  
- **Normal-tolerance parts** (PN-B, standard): 8-12% contamination  
  - Moderate natural variation, captures real defects
  
- **Loose-tolerance parts** (PN-C, commodity): 12-18% contamination
  - High natural variation, detects significant outliers only

**How the decision is made:**
The app uses 4 statistical methods (in order of preference):
1. **Elbow Method** - Finds natural gaps in anomaly scores
2. **Z-Score Detection** - Identifies scores >1.5σ from mean
3. **IQR Method** - Uses Tukey's quartile-based outlier detection
4. **Fallback Heuristic** - Based on sample count per type

### The Analysis Process

**Step 1: Column Mapping**
- **Part ID:** Unique identifier for each sample (e.g., serial number, part number, row ID)
- **Part Number/Label:** The declared product type (e.g., PN-A, PN-B)
- **Features:** 3+ numeric columns used to detect anomalies
  - Measurements (length, width, weight, etc.)
  - Process parameters (temperature, pressure, speed, etc.)
  - Any numeric data that characterizes the part

**Step 2: Data Quality Check**
- Column completeness analysis shows which columns have the most data
- Missing values are removed (you'll see how many rows are affected)
- Only parts with all selected features are analyzed

**Step 3: Smart Contamination Detection**
- For EACH part type, analyze the distribution of anomaly scores
- Choose optimal contamination rate using statistical methods
- Display why each rate was chosen (which method won)

**Step 4: Anomaly Detection**
- For each part type, fit an IsolationForest model with its optimal contamination rate
- Assign anomaly scores (0-100, where 100 = most anomalous)
- Flag parts as "Anomaly" or "Normal"

**Step 5: Results & Explanation**
- View all results or just anomalies
- Select any part to see which features caused the flag
- Examine Z-scores (how many std devs from the type's mean)
- Review distribution charts and visualizations

### Understanding the Results

**Anomaly Score (0-100)**
- **0-20:** Very normal, typical of its type
- **20-40:** Slightly unusual but within normal range
- **40-60:** Notably different from typical samples
- **60-80:** Highly suspicious, likely mislabeled or defective
- **80-100:** Extremely anomalous, almost certainly wrong

**Z-Score Explanation**
When you select a part to explain, you see Z-scores for each feature:
- **|Z| < 1:** Feature is normal (within 1 std dev of type mean)
- **|Z| 1-2:** Somewhat unusual (1-2 std devs away)
- **|Z| 2-3:** Very unusual (2-3 std devs away)
- **|Z| > 3:** Extreme outlier (more than 3 std devs away)

Higher Z-scores = features that made the part flagged as anomalous

### How Contamination Rates Are Determined

The app automatically selects a contamination rate (% of expected anomalies) for each part type based on four statistical methods:

**Method 1: Elbow Detection**
- Looks for natural gaps in anomaly scores
- Most accurate when data has clear clusters
- Indicates where normal transitions to anomalous

**Method 2: Z-Score Based**
- Finds scores >1.5 standard deviations from the mean
- Statistically rigorous approach
- Works well for normally distributed data

**Method 3: IQR (Interquartile Range)**
- Uses Tukey's outlier detection formula (Q3 + 1.5×IQR)
- Robust to extreme values
- Conservative approach

**Method 4: Sample-Count Fallback**
- <20 samples → 15% contamination
- 20-99 samples → 10% contamination
- 100+ samples → 8% contamination

The app selects whichever method produces the most reliable result for your data.

### Common Questions

**Q: Why do different part types have different contamination rates?**
A: Each part type has different natural variation. A precision bearing (tight tolerance) naturally has much less variation than a commodity part (loose tolerance). The app detects this and adjusts the threshold accordingly.

**Q: Can I use this to find mislabeled parts?**
A: Yes! If a part has characteristics more typical of PN-B but is labeled as PN-A, it will be flagged as anomalous. The explanation will show which features don't match its declared type.

**Q: What if I have very few samples of a part type?**
A: If a part type has fewer than 2 samples, anomaly detection can't work (you need variation to detect anomalies). The app will warn you about these. Collect more samples or combine related part types.

**Q: How do I know if the contamination rate is right?**
A: Look at the anomaly score distribution chart. The red line (threshold) should be in a gap or at a natural separation. If it looks like it's in the middle of the data, the threshold may be too aggressive or too lenient.

**Q: What do the visualizations tell me?**
- **Scatter Plot:** See if normal and anomalous parts cluster separately
- **PCA:** View all features compressed to 2 dimensions
- **Boxplot:** See the range of values for the main feature by part type
- **Correlation Heatmap:** Check which features are related to each other

### Best Practices

1. **Start with demo data** to understand how the system works
2. **Review the column quality analysis** before selecting features (prefer >90% complete)
3. **Trust the auto-detection** for contamination rates (it uses 4 statistical methods)
4. **Examine the distribution chart** to see why the threshold was chosen
5. **Check at least one anomaly** explanation to understand the flags
6. **Compare with your domain knowledge** - does it match what you expect?
7. **Export results** and share with your team for validation
8. **Iterate** - review results and refine your feature selection if needed

### Technical Details

**Anomaly Detection Method:** IsolationForest
- Isolates anomalies by randomly selecting features and values
- Doesn't assume normal distribution
- Works well with multiple features
- Robust to outliers

**Per-Type Analysis:** 
- Each part type is analyzed separately
- Allows different contamination rates for each type
- Prevents tight-tolerance parts from skewing loose-tolerance analysis (and vice versa)

**Missing Data Handling:**
- Any row with missing values in selected features is removed
- You'll see how many rows are dropped and why
- Only complete cases are analyzed

---
**Questions or issues?** Contact: **chad@hertzler.com**
"""
    )
...
if st.session_state.df_analysis is None:
    ...
    # ===== ANOMALY DETECTION (WITH OPTIMAL CONTAMINATION) =====
